...
- `calculator` previously used Python's `eval()`. Replaced with an AST-based safe evaluator that only permits whitelisted arithmetic operators and functions from Python's `math` module — arbitrary code execution is no longer possible.
